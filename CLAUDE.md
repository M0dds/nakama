...
**Stand (Kurzfassung):** App ist **live auf usenakama.app** (v0.9.0, Git-Auto-Deploy aus `main`). Phasen 1-9 + alle Medien-Themen (1a Serien · 1b Filme · 1c Spiele/Steam) + Sync-Instanzen + Cover-Epos + Push Phase 1 sind live; 28 Migrationen gefahren; Security-Audit durch. Aktiver Backlog: `FEEDBACK-BACKLOG.md` (R1-R3 deployed, **nächste Session bei R4**). **Der vollständige Stand + die nächsten Schritte leben an genau einer Stelle: `handshake.md` §Stand · §Status · §Offene Punkte** (+ `FEEDBACK-BACKLOG.md`). Hier NICHT duplizieren — sonst driftet's auseinander.

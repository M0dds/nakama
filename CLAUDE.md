...
**Stand (Kurzfassung):** Phasen 1-8 + Sync-Instanzen + Politur-Session **+ alle Medien-Themen 1a (Serien) · 1b (Filme) · 1c (Spiele/Steam)** sind in lokalem `main`. Außerdem: Detailseiten-Politur, Confirms global als `ConfirmDialog`, Layout-Content-Frame (1728px Cap + Framing-Hairlines). Arbeit geht direkt auf `main` (User-delegiert, atomare Commits). `origin` ist bewusst nicht aktuell („mergen ok, nur nicht pushen"). Als Nächstes (frische Session): **Themen 2-4** — „Weitere laden"→Paging · First-Login-Setup · Onboarding-Tooltips (Spec in `handshake.md` §Offene Punkte). **Der vollständige Stand + die nächsten Schritte leben an genau einer Stelle: `handshake.md` §Stand, §Status, §Offene Punkte.** Hier NICHT duplizieren — sonst driftet's auseinander.

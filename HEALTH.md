...
- **PRELAUNCH-1 (offen, kein Leck)** — `items` + `episodes` welt-schreibbar (`INSERT/UPDATE using true`): jeder eingeloggte User kann Katalog ändern. Vor echtem Launch eindämmen (z.B. nur via DEFINER-RPC schreiben).
- **PRELAUNCH-2 (offen, Rechte-Breite)** — `list_members`-DELETE + `lists`-UPDATE erlauben jedem Mitglied, andere Nicht-Owner zu entfernen / die Liste umzubenennen. Passt zum equal-members-Modell; bewusst bestätigen oder owner-only machen.
- **PRELAUNCH-3 (kosmetisch)** — redundante doppelte SELECT-Policies auf `item_history` + `profiles` (alte own-only neben co-member, OR-verknüpft). Aufräumbar.

...
- **Migrationen** fährt der User manuell im Supabase SQL-Editor. Bei neuer Migration im Chat ankündigen + den SQL liefern (**Falle:** der User kopiert leicht den Erklärtext mit → SQL klar abgrenzen). Eine frische Nakama-DB = erst Logbook-Migrationen (leben in dessen Repo), dann Nakamas Files in Timestamp-Reihenfolge. **Status: alle 27 gefahren + User-bestätigt.** Liste (ID — Zweck):

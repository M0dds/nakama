...
| **8 · Polish** | ✓ done — Cover/Avatar-Fade-in (`fadeOnLoad`), Skeleton-States (`Skeleton`), Theme-Switch-Crossfade (`@layer base`). Route-Transitions zunächst verworfen, dann 2026-06-02 auf User-Wunsch dezent ergänzt (`RouteTransition` — §Gotchas Motion) |
...
- **Route-Transition (`RouteTransition`, 2026-06-02).** Ursprünglich bewusst hart (Tab-Tool wie iOS-Tabbar); auf expliziten User-Wunsch dann doch ergänzt — aber als **einzelner Container-Level-Enter** (kein Per-Element-Choreo, das bleibt verworfen). Liegt in der AppShell um `props.children`, spielt bei jedem `location.pathname`-Wechsel eine kurze WAAPI-Animation (opacity 0→1 + `top` 10px→0, back-out-Easing `cubic-bezier(0.34,1.5,0.5,1)` = dezenter Bounce). **`top`/`position:relative`, NICHT `transform`** — ein Transform erzeugt einen Containing-Block für `position:fixed`-Kinder und würde die seiten-interne, fixe `ColumnGuide` einsperren/verschieben (der #4-Portal-Fallstrick); `top` lässt die Guide viewport-fixiert, der Content „setzt sich" beim Reinkommen hinein. `on(…)` (defer) überspringt den ersten Run → Hard-Reload/Deep-Link animiert NICHT, nur echte In-App-Navigation. reduced-motion-aware.

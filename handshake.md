...
**Stand (2026-06-02):** Phasen 1-8 + Sync-Instanzen + Politur + alle Medien-Themen (1a Serien · 1b Filme · 1c Spiele/Steam) + First-Login-Setup + Cover-Epos sind in lokalem `main`. Build grün, Tree clean, **22 Migrationen** (alle gefahren + User-bestätigt — Liste → §Gotchas → Daten/RLS). Arbeit läuft über kurzlebige Feature-Branches → `main` (atomare Commits, „better safe than sorry"); **`origin` bewusst NICHT aktuell** (~200 Commits voraus, „mergen ok, nur nicht pushen"). **Thema 4 (Onboarding-Tooltips) verworfen** — Empty-Canvas-Onboarding löst es besser (§Offene Punkte). **Nächstes:** Feature-Backlog-Rest (#4–#15) + Launch-Runway (Phase 9 / E-Mail-Prod) → §Offene Punkte.
...
Vom 16-Punkte-Backlog des Users erledigt: #14 (Tracken-Bug), #8/#9 (Sakura/Dialoge), #1/#2/#3 (Cover-Epos), **#4/#7/#10/#12 (Quick-Wins Batch 2)**, **#6 („Was kommt" mobil)**. **Offen, grob nach Aufwand:**
...
- **Migrationen** fährt der User manuell im Supabase SQL-Editor. Bei neuer Migration im Chat ankündigen + den SQL liefern (**Falle:** der User kopiert leicht den Erklärtext mit → SQL klar abgrenzen). Eine frische Nakama-DB = erst Logbook-Migrationen (leben in dessen Repo), dann Nakamas Files in Timestamp-Reihenfolge. **Status: alle 22 in `supabase/migrations/` gefahren + User-bestätigt.** Liste (ID — Zweck):

...
1. **Serien · Filme · Spiele integrieren.**
   ...
   - **1b · Filme (TMDB) — ✓ erledigt (in `main`), bis auf „Was kommt".** Binärer Gesehen-Status (`item_history` `completed` an/aus, kein `'planned'` → keine CHECK-Migration), eigene Film-Detailseite, Mitseher-Auge via Co-Member-RLS. Detail → §Status-Zeile *Thema 1b*. **Rest = Scheibe 3:** unreleased Filme in „Was kommt" (Home). Der **deutsche** Release liegt in `fetchTmdbMovieDetails.releaseDate`; `items.metadata.releaseDate` wird beim Hinzufügen aus der Suche (US-primary) gestempelt — für „Was kommt" entweder beim Detail-Besuch mit dem DE-Datum backfillen oder pro Kandidat nachladen (in Scheibe 3 entscheiden).
   - **1c · Spiele (Steam) — offen, als Nächstes nach Scheibe 3.** Steam-Store-Endpoints (`store.steampowered.com/api/storesearch` + `/appdetails`, kein API-Key, kein OAuth) — **aber CORS-geblockt → braucht einen Supabase Edge-Function-Proxy** (Deno). `items.source` braucht **Migration** (`'steam'` fehlt in der CHECK; `'igdb'` wäre erlaubt, nehmen wir aber nicht). Status-UI aus 1b wird wiederverwendet. (SteamDB.info selbst geht NICHT — Cloudflare/kein API.)

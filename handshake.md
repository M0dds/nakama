...
**Stand (2026-06-02):** Phasen 1-8 + Sync-Instanzen + Politur + alle Medien-Themen (1a Serien · 1b Filme · 1c Spiele/Steam) + First-Login-Setup + Cover-Epos sind in lokalem `main`. Build grün, Tree clean, **24 Migrationen** (23 gefahren + bestätigt; `20260603100000` neu — **noch zu fahren**; Liste → §Gotchas → Daten/RLS). Arbeit läuft über kurzlebige Feature-Branches → `main` (atomare Commits, „better safe than sorry"); **`origin` bewusst NICHT aktuell** (~200 Commits voraus, „mergen ok, nur nicht pushen"). **Thema 4 (Onboarding-Tooltips) verworfen** — Empty-Canvas-Onboarding löst es besser (§Offene Punkte). **Nächstes:** Feature-Backlog-Rest (#4–#15) + Launch-Runway (Phase 9 / E-Mail-Prod) → §Offene Punkte.
...
- **Migrationen** fährt der User manuell im Supabase SQL-Editor. Bei neuer Migration im Chat ankündigen + den SQL liefern (**Falle:** der User kopiert leicht den Erklärtext mit → SQL klar abgrenzen). Eine frische Nakama-DB = erst Logbook-Migrationen (leben in dessen Repo), dann Nakamas Files in Timestamp-Reihenfolge. **Status: 23 von 24 gefahren + User-bestätigt; `20260603100000` (item_notes) ist neu und NOCH ZU FAHREN.** Liste (ID — Zweck):

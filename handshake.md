...
### Nächste Session — Themen 3-4 (vom User priorisiert; vor Umsetzung Detail-Design + bei Schema/Screens fragen)

*(Thema 2 · Paging ist erledigt → §Diese Session erledigt.)*

1. **First-Login-Einrichtung.** Beim allerersten Login *vor* der Startseite ein Setup-Fenster: Anzeigename + Profilbild + Theme + **@handle (wählbar, User-Entscheidung)**. Handle wird vorgeschlagen (Discord-Username bzw. Mail-Lokalteil) und ist editierbar — **jeder** darf den Handle bearbeiten (nicht nur Magic-Link-User). Braucht: **`updateUsername`-Pfad** (existiert noch nicht; `profiles.username` ist `UNIQUE` → Live-Verfügbarkeitscheck + `23505`-Race im UI abfangen) und das **„onboarded"-Flag** (DB-Spalte auf `profiles`, z.B. `onboarded_at` — Schema-Migration, ich liefere SQL) + Route-Guard, der bis dahin auf `/setup` lenkt. Bausteine: `EditableDisplayName`, `EditableAvatar`/`AvatarCropDialog`, `ThemeSwitcher`. Theme-Default für First-Timer ist bereits Standard+System.
2. **Onboarding-Tooltips.** Geführte Einführung über die `Tooltip`-Primitive, **nur beim ersten Login**, **überspringbar**. Teilt die First-Login-Erkennung mit dem First-Login-Setup (gemeinsames Flag). Reihenfolge/Skip-State persistieren.

...
- **E-Mail-Prod (Auth) → NÄCHSTER SCHRITT, volle Anleitung in `MAGIC-LINK-PREP.md`.** Magic-Link-Code ist fertig + prod-tauglich (`signInWithMagicLink` → `signInWithOtp`, `emailRedirectTo` = `…/auth/callback`); offen ist nur die **Infra**: Custom SMTP über eine **verifizierte Resend-Domain** (sonst nur ~4 Mails/Std über den Supabase-Mailer), Supabase-URL-Config + Templates, „Confirm email"-Entscheidung, Prod-Test. Discord-`email`-Scope + Same-Email-Linking sind verifiziert; `usenakama.app` hat seit 2026-06-18 **Cloudflare Email Routing** (nur Empfang: `servus@` → Gmail) — Resend macht den **Versand**. SPF-Koexistenz beachten (→ Prep-Doc).

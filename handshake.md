...
**Stand (2026-07-06):** App ist **live auf usenakama.app** (Cloudflare Workers, **Git-Auto-Deploy aus `main`**; gelegentliche reine Docs-Commits reiten bewusst mit dem nächsten echten Deploy mit, weil ein Solo-Push über `__GIT_SHA__` ein Fehl-Update-Badge auslöst, siehe Memory `git-sha-triggers-update-toast`). Aktuelle Version **v0.18.0**. **Live:** Phasen 1-9 · alle Medien-Themen (1a Serien · 1b Filme · 1c Spiele/Steam) · Sync-Instanzen · First-Login-Setup · Cover-Epos · Pre-Launch-Features · **Push Phase 1+2** (Auto-Push bei neuen Folgen via pg_cron/Edge-Function) · **E-Mail-Prod** (Magic-Link über Resend-SMTP, seit 2026-06-18) · **Media-Proxy** (Cloudflare-Worker `/api/media/*`, TMDB-Token server-seitig, v0.15.0). **36 inkrementelle Migrationen** (+ `00000000000000_baseline`), alle gefahren + bestätigt (Liste → §Gotchas → Daten/RLS). Security-Audit durch (Report `SECURITY-AUDIT.md`); `FEEDBACK-BACKLOG.md` (18 Punkte R1-R5) komplett deployed. **Seit v0.15:** **v0.16.0 App-Redesign** (2026-06-30: true-sharp corners, tokenisiertes `.glass`, `CoverBackdrop`-Ambient-Layer auf allen eingeloggten Flächen) · **v0.17.0** (2026-07-01: Generated-Cover-System neu — weiche theme-entkoppelte Paletten + Reroll; Member-Avatare auf der Übersicht) · **Monats-Review 2026-07** (`REVIEW-2026-07.md`, 54 verifizierte Findings + Produkt-Roadmap): **P0 komplett** (lane-aware Row-Aktionen · Leave-Merge-RPCs + Trigger + Heal, Migration `20260703100000` · Magic-Link-Fehlerpfad) und **P1 komplett** (Touch-Paket: „⋯"-Row-Toggle + immer sichtbarer Drag-Griff auf Coarse-Pointern, sichere pointer-events-Gates · `QueryErrorCard` an den 5 Query-Gates · Dialog-Fokus · C3/C4-Effect-Fixes · W1 Fortsetzen-Mobile-Row) · **v0.18.0 Mobile-Session** (2026-07-05/06: **AddSheet-Suche mobil TOP-verankert** — Tastatur-Auto-Open per Warm-up-Input, Fokus bleibt beim Hinzufügen, echte Body-Scroll-Lock; Grund: iOS-26-visualViewport-Bug + fehlende VirtualKeyboard-API in Safari, Details als Kommentare in `AddSheet.tsx` · **CoverBackdrop als Canvas-Bake** statt Vollbild-`blur(80px)` — fixt 1-2s-Freezes pro Tab-Wechsel in der installierten PWA · globale ≥16px-Input-Regel gegen iOS-Auto-Zoom (`index.css`) · Update-Reload mit Spinner-Feedback). **Offen:** Review-**P2** (Hygiene: Worker-AniList-Rest C5 + **Docs-Pass C6** — §Gotchas braucht die neuen iOS-/Backdrop-Erkenntnisse + Redesign-/Cover-Details, CLAUDE.md-Sync) und **P3** (Produkt-Roadmap §4, „v0.18/v0.19 — Der Was-jetzt-Release": Als-Nächstes-Strip · Abschluss-Momente · Seasonal-Break-Limbo · Notizen ins Logbuch · Discovery-Seed · Partner-Delta · Heute-Abend-Nudge) + Impressum-Aktivierung → §Offene Punkte.
...
**`REVIEW-2026-07.md`** (Monats-Review, 54 verifizierte Findings + Produkt-Roadmap) ist der aktive Plan. Stand 2026-07-06: **P0 ✓** (C1/C2/U2/C7, Migration `20260703100000` gefahren) · **P1 ✓** (U1-Touch-Paket, U3-Fehler-Karten, U4-Dialog-Fokus, C3/C4, W1) · dazu außerplanmäßig die **Mobile-Session v0.18.0** (AddSheet-Suche mobil neu, Backdrop-Perf, Update-Feedback — siehe §Stand). **Als Nächstes: P2** (C5 Worker-AniList-Rest raus · **C6 Docs-Pass**: §Gotchas um iOS-Keyboard-/Backdrop-Bake-/Redesign-/Cover-Wissen ergänzen, CLAUDE.md-Sync, CODE-HEALTH-AUDIT-Tracking · Low-Listen nach Gusto) **dann P3** (Produkt-Roadmap §4 in Reihenfolge — #1 „Als Nächstes"-Strip zuerst).

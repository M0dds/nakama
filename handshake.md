...
**Stand (Tagesstatus):** Phasen 1-8 + Sync-Instanzen + Politur-Session + **Themen 1a (Serien) · 1b (Filme) · 1c (Spiele/Steam)** alle in lokalem `main`. Arbeit läuft seit 2026-06-01 über **kurzlebige Feature-Branches → `main` gemerged** (User: „better safe than sorry"; atomare Commits). **`origin` bewusst NICHT aktuell:** `main` ist `origin/main` ~200 Commits voraus — „mergen ok, nur nicht pushen". Build grün, Tree clean, **22 Migrationen** in `supabase/migrations/` (Status je Migration → §Gotchas → Daten/RLS). Feature-Detail je Phase → §Status; Nächstes → §Offene Punkte. **Alle drei Medien-Themen durch:** Serien (echte Staffeln), Filme + Spiele (episodenlos → binärer Gesehen/Gespielt-Status in `item_history`, eigene Detailseite, Mitseher-Auge, unreleased in „Was kommt"). **Außerdem:** `ConfirmDialog` (Modal statt inline); Detailseiten-Politur; Content-Cap als feste Breite (1728px) + Framing-Hairlines. **Zuletzt (2026-06-01):** Health-Audit (23-Agenten-Review → 10 Fixes, HEALTH Bundle 8; u.a. `added_by_user_id`-Bug) **+ Thema 2 · Paging** (nummerierter `Pager` + geteilter `createLiquidBubble`-Hook, den Pager & `Segmented` nutzen) **+ Login-Redesign** (Discord-Hero, Magic-Link hinter „Kein Discord-Konto?"-Disclosure, `email`-Scope) **+ `UserChip`** (Hover-Identitätskarte Avatar+Anzeigename+@handle in Logbuch/Roster/Mitseher — Anti-Spoofing). **Auth-Modell:** Discord primär; Magic-Link an dieselbe Discord-Mail = Recovery in *denselben* Account (Supabase Same-Email-Linking, verifiziert). **+ Thema 3 · First-Login-Setup** (`/setup`-Wizard Identität→Avatar→Theme, Handle wählbar mit Live-Check, `onboarded_at`-Gate; Migr. `20260601100000`). **+ Privacy-Fix + Audit:** ein **echtes RLS-Leck** gefunden & geschlossen (globale `episode_watches` waren für jeden sichtbar, der dasselbe Item trackt — `is_co_member` band nicht an den Watcher; Migr. `20260601110000`), dann **alle** Lese-Pfade auditiert (RLS-Policies + DEFINER-RPCs) → **sauber**. **+ Empty-Canvas-Onboarding** (Migr. `20260601120000`): Auto-„Watchlist" beim Signup entfernt → neue User starten leer; Home-Empty-States nun **kontextabhängig** (frischer User → erklärt Sektion + Link in den Listen-Tab; eingespielt-aber-leer → ruhige Copy), AddSheet/`PrivateEmpty` angeglichen. **Thema 4 (Onboarding-Tooltips) damit verworfen** — kontextuelle Empty States ersetzen die Tour (§Offene Punkte). **+ Pre-Launch-RLS-Härtung** (Migr. `20260601130000`, HEALTH Bundle 9): offener Launch → `items`/`episodes`-Writes über DEFINER-RPCs (`upsert_item`/`set_item_metadata`/`upsert_episodes`) + direkte Table-Writes entzogen; `lists`-Rename + `list_members`-Entfernen **owner-only** (UI: Pencil + neuer „Mitglied entfernen"-Button owner-gated); `item_history`-Co-Read auf „Item co-präsent in geteilter Liste" verschärft (AUD-10). **+ Session 2026-06-02 (Feature-Backlog):** **Tracken/Archiv-Fix** (Mitglied konnte geteilte Liste nicht archivieren — owner-only RLS auf direktem `list_members`-UPDATE; Fix: DEFINER-RPC `set_list_tracking`, Migr. `20260601140000`; dabei **latenter Leak** gefunden+gefixt: `trackedItemIds` scopte nicht auf den eigenen `user_id`, daher zog die `tracks_home`-Wahl eines Co-Members Items in *fremde* Homes/Kalender) **+ Visual Quick-Wins** (Sakura Light heller/mehr Kontrast, Dark-Mode-Dialoge auf `dark:bg-surface` gehoben; **#10 Grain vertagt** — aktuelles Korn ist retina-subpixel-fein → gröberes `baseFrequency` + Opacity-Tuning offen) **+ Cover-Epos (#2/#1/#3):** `GeneratedCover` (Seed → zufälliges Theme + japanisch-geometrisches Muster Seigaiha/Shippō, Inline-SVG, kein Storage, re-coloriert per Mode via `useResolvedMode`), Custom-Cover owner-upload (`list-covers`-Bucket, quadratischer Crop über `AvatarCropDialog shape="square"`), Cover quadratisch in Übersicht + 1/3 in Detail; Pin-Status als **`PinBadge`** auf dem Cover (Row-Pin-Icon jetzt hover-only, orange wenn aktiv); Item-Cover in Listen **hochkant (2:3)**; `fadeOnLoad` dedupt eingefadete `src` (kein Re-Fade-Flacker beim `<For>`-Remount). Migr. `20260601150000`+`20260601160000`. **Nächstes: Feature-Backlog-Rest** (§Offene Punkte) **+ Launch-Runway** (E-Mail-Prod / Phase 9).
...
- **Empty-Canvas-Onboarding** (Branch `feat/empty-canvas`): Auto-„Watchlist" beim Signup entfernt (`handle_new_user()` ohne Listen-Insert, **Migr. `20260601120000`** — Profil-Insert 1:1 erhalten, Alt-Watchlists unberührt) → neue User starten auf leerem Canvas. Home-Empty-States **kontextabhängig** via `firstRun` (eigene Listen-Query): frischer User (keine Liste) → erklärt die Sektion + Link in den **Listen-Tab**; eingespielt-aber-leer → ruhige „Diese Woche ruhig / Alles aufgeholt"-Copy (beide ohne den alten „Watchlist"-Verweis). AddSheet-Zero-Lists-Fallback + `/lists` `PrivateEmpty` auf dieselbe Führung + die zentrierte Hairline-Box-Optik der Dashboard-States angeglichen.

### Session 2026-06-02 erledigt (in `main`)

- **Tracken/Archiv-Bug** (Branch `fix/member-tracking-toggle`, gemerged): ein Mitglied konnte eine geteilte, getrackte Liste nicht auf „Archiv" stellen — der Toggle sprang zurück. Ursache: `list_members`-UPDATE ist owner-only (`list_members_update_owner`), der direkte `.update()` wurde still RLS-geblockt. Fix: DEFINER-RPC **`set_list_tracking`** (self-scoped, spiegelt `set_list_pin`), Migr. `20260601140000`; `setListTracking` → `.rpc()`. **Dabei latenter Leak gefixt:** `trackedItemIds` filterte `tracks_home=true` ohne `.eq("user_id", …)` und verließ sich auf RLS — die `list_members`-SELECT-Policy gibt aber auch **Co-Member-Zeilen** zurück (Roster). Folge: in *jeder* geteilten Liste zog ein Co-Member, der trackt, ihre Items in *dein* Home/Kalender/Logbuch, auch wenn du archiviert hast. Jetzt explizit auf den Aufrufer gescopt. Toggle-Copy erklärt jetzt Tracken/Archiv (vorher „Off bedeutet …", obwohl es kein „Off" gibt). Toggle invalidiert Home+Kalender (sonst greift Archiv erst nach `staleTime`).
- **Visual Quick-Wins** (Branch `feat/quick-wins-visual`, gemerged): #8 Sakura Light heller (`--bg #fbe7ee`, weiße Cards) für mehr Kontrast; #9 Modal-Dialoge im Dark Mode von `bg-bg` (fast-schwarzer Canvas) auf `dark:bg-surface` gehoben (`ConfirmDialog`/`MoveItemDialog`/`AvatarCropDialog`, Inner-Hover weißer Overlay). **#10 Grain vertagt** (das Korn ist retina-subpixel-fein → unsichtbar; Fix wäre gröberes `baseFrequency` (~0.65) + entsättigt + Opacity ~0.09/0.13 — war gebaut, auf User-Wunsch zurückgerollt für später).
- **Cover-Epos (#2/#1/#3)** (Branch `feat/list-covers`, gemerged): siehe §Status-Zeile *Cover-Epos* + §Gotchas → List Covers. Migr. `20260601150000` (Spalten+Bucket) + `20260601160000` (Storage-Policy-Namenskonflikt-Fix).
...
- **Migrationen** fährt der User manuell im Supabase SQL-Editor. Bei neuer Migration im Chat ankündigen + den SQL liefern (Falle: der User kopiert leicht den Erklärtext mit — SQL klar abgrenzen). Seit 2026-05-29 in `supabase/migrations/` getrackt: Phase-3-5-Catch-up `20260528200000`, Home-RPCs `20260529120000`, Pin-RPCs `20260529130000`, Auto-Sync-Cascade `20260530120000`, Realtime-Sharing-Tables `20260530140000`, Avatar-Storage `20260530150000`, delete_account `20260530160000`, invitation_names_prefer_display `20260530170000` (**Status prüfen — war zuvor offen**), **Sync-Instanzen `20260531100000` (gefahren), home_sync_instances `20260531120000` (gefahren), unshare_when_solo `20260531140000` (gefahren), reset_progress_fanout `20260531160000` (gefahren — synced Reset fächert für alle Mitglieder), continue_watching_seasons `20260531180000` (gefahren), item_history_co_member `20260531200000` (gefahren — `item_history_select_co` via `shares_list_with` für das Film-Mitseher-Auge + item_history in die supabase_realtime publication)**, **items_source_steam `20260531210000` (gefahren — `'steam'` in den `items.source`-CHECK für Thema 1c)**, **onboarding `20260601100000` (gefahren — `profiles.onboarded_at` + Backfill + `username_available`-RPC für Thema 3)**, **fix_episode_watches_global_privacy `20260601110000` (gefahren — globale-Lane-RLS auf `shares_list_with(user_id,…)` statt `is_co_member`, schließt Cross-User-Leck)**, **remove_default_watchlist `20260601120000` (`handle_new_user()` ohne Listen-Insert — neue User starten leer; Profil-Insert 1:1, Alt-Watchlists unberührt)**, **prelaunch_hardening `20260601130000` (gefahren — Pre-Launch: DEFINER-RPCs `upsert_item`/`set_item_metadata`/`upsert_episodes` + Direkt-Writes auf items/episodes entzogen; `lists`-UPDATE + `list_members`-DELETE owner-only; `item_history_select_co` via neuem `shares_item_in_list_with`; `item_history_select_own` entfernt)**, **set_list_tracking `20260601140000` (gefahren — DEFINER-RPC, mit der ein Mitglied seine eigene `tracks_home`-Zeile togglen kann; das direkte UPDATE scheiterte an der owner-only `lists_update_owner`/`list_members`-Policy)**, **list_covers `20260601150000` (gefahren — `lists.cover_url` + `lists.cover_seed` (DB-Default random) + `list-covers`-Storage-Bucket, public read / owner-only write)**, **fix_list_cover_policies `20260601160000` (gefahren — Storage-Write-Policy qualifiziert den Objektpfad als `storage.objects.name`; unqualifiziertes `name` band im Subquery an `lists.name` → owner-check immer falsch → 403 für alle, siehe §Daten/RLS)** = 22 Files. **Status: alle bis `20260601160000` gefahren + User-bestätigt; `20260601120000` (remove_default_watchlist) inzwischen ebenfalls bestätigt durch (neue Signups bekommen keine Auto-„Watchlist" mehr).** Logbook-Era-Schema lebt weiter in dessen Repo; eine frische Nakama-DB = Logbook-Migrationen zuerst, dann Nakamas Files in Timestamp-Reihenfolge.

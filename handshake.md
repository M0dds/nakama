...
**Stand (Tagesstatus):** Phasen 1-8 + Sync-Instanzen + Politur-Session + **Themen 1a (Serien) · 1b (Filme) · 1c (Spiele/Steam)** alle in lokalem `main`. Alles **direkt auf `main`** (User-delegiert, atomare Commits; kein Feature-Branch-Flow). **`origin` bewusst NICHT aktuell:** `main` ist `origin/main` ~190 Commits voraus — „mergen ok, nur nicht pushen". Build grün, Tree clean, **15 Migrationen** in `supabase/migrations/` (Status je Migration → §Gotchas → Daten/RLS). Feature-Detail je Phase → §Status; Nächstes → §Offene Punkte. **Alle drei Medien-Themen durch:** Serien (echte Staffeln), Filme + Spiele (episodenlos → binärer Gesehen/Gespielt-Status in `item_history`, eigene Detailseite, Mitseher-Auge, unreleased in „Was kommt"). **Außerdem diese Session:** alle Confirms laufen jetzt über einen einheitlichen `ConfirmDialog` (Modal statt inline); Detailseiten-Politur (Release auch in Details · mobil Cover zuerst) erledigt; **Layout: Content-Cap als feste Breite (1728px) + Framing-Hairlines + full-bleed Topbar-Rule**. **Nächste Session: Themen 2-4** — „Weitere laden"→Paging · First-Login-Setup · Onboarding-Tooltips (§Offene Punkte).

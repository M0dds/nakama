...
**Stand (Tagesstatus):** Phasen 1-8 + Sync-Instanzen + Politur-Session + **Thema 1a (TMDB-Serien)** alle in lokalem `main`. Alles **direkt auf `main`** (User-delegiert, atomare Commits; kein Feature-Branch-Flow). **`origin` bewusst NICHT aktuell:** `main` ist `origin/main` ~173 Commits voraus — „mergen ok, nur nicht pushen". Build grün, Tree clean, **14 Migrationen** in `supabase/migrations/` (Status je Migration → §Gotchas → Daten/RLS). Feature-Detail je Phase → §Status; Nächstes → §Offene Punkte. **Thema 1a (Serien) + 1b (Filme) sind durch** (Film-Detailseite, binärer Gesehen-Status, Mitseher-Auge, unreleased Filme in „Was kommt" nach DE-Release). **Nächste Session: Detailseiten-Politur** (Release zusätzlich in Details · mobil Cover/Details vor Episoden + Nummern tauschen — siehe §Offene Punkte „Nächste Session"), dann 1c (Spiele/Steam) bzw. Themen 2-4.
...
| **Thema 1a · Serien (TMDB)** | ✓ **in `main`** — 2. Quelle TMDB (`tmdb.ts`, browser-side, `VITE_TMDB_TOKEN`), quellen-agnostische Suche (`search.ts` + Medientyp-Filter im AddSheet), echte Staffeln + Staffel-Gliederung, Continue-Watching staffel-bewusst (Migr. `20260531180000`), date-only Air-Dates ohne erfundene Zeit, Badge episoden-gezählt + tracking-gated. Spiele (1c) noch offen — §Offene Punkte. |
| **Thema 1b · Filme (TMDB)** | ✓ **in `main`** (bis auf „Was kommt", s.u.) — Film-Suche (`searchTmdbMovies`), Film-Filter im AddSheet. Filme sind **folgenlos** → binärer Gesehen-Status in `item_history` (`status='completed'` an/aus, `queries/status.ts`), **keine** items/CHECK-Migration. **Eigene Film-Detailseite** (Movie-Branch in `ItemDetail`, kein Episoden-Listing): links Gesehen-Toggle (Episoden-Tick-Optik) + Release-Datum mit Heute/Morgen/Demnächst-Tag (≤14 Tage) + Tagline + Beschreibung + Besetzung (Foto-Streifen, auto-fit-Grid); rechts „Details" mit Regie/Laufzeit/Genres/FSK/Quelle. Film-Infos **live aus TMDB** (`fetchTmdbMovieDetails`, kein DB-Store), **deutscher** Release (`release_dates`, Kino bevorzugt). **Mitseher-Auge** in geteilten Listen über `item_history`-Co-Member-RLS (Migr. `20260531200000`). **Unreleased Filme in „Was kommt"** nach DE-Release (`items.metadata.releaseDate`, beim Add gestempelt + auf der Detailseite per `setItemReleaseDate` nachgezogen). Offene Detailseiten-Politur → §Offene Punkte. |
...
### Nächste Session — Detailseiten-Politur (`/item/:type/:slug`, alle Medientypen)

Vom User für eine frische Session zurückgestellt (nicht angefangen). Beides in `src/routes/ItemDetail.tsx`:

1. **Release-Datum ZUSÄTZLICH in „Details" (rechts).** Steht aktuell nur in der „Gesehen"-Zeile links (mit Heute/Morgen/Demnächst-Tag). User will es zusätzlich als Fact in der rechten Details-Spalte (`MovieFacts`), falls man's oben übersieht. Label „Kinostart" (Datum künftig) bzw. „Erschienen" (Vergangenheit), **volle Datumsform mit Jahr**. Falle: der `releaseLabel`-Helper (volles Datum m. Jahr) wurde in `c60d8db` entfernt → neu anlegen/inlinen. Gesehen-Zeile NICHT anfassen (Datum bleibt dort *auch*).
2. **Mobil: Section 02 (Details/Cover) VOR Section 01 (Episoden/Film) + Nummern tauschen.** Motivation des Users: „das Cover nicht ganz oben zu sehen ist komisch" — gilt für Filme, Serien, Anime (= alle Typen, ein Component). Auf `< md`: Details-Spalte (mit Cover) zuerst, darunter Episoden/Film; die Bento-**Nummern** sollen mitspringen, sodass das zuerst Gezeigte „01" trägt (mobil Details=01/Episoden=02; ab `md` wie heute: Episoden 01 links, Details 02 rechts). Touchpoint: der `<div class="flex flex-col md:flex-row …">` mit den zwei `<BentoModule label number>`. **Knackpunkt:** `BentoModule.number` ist ein statischer String → reine CSS-`order`-Umsortierung tauscht die Nummer NICHT mit. Optionen: (a) `BentoModule` um responsive Nummer erweitern (zwei Spans `md:hidden` / `hidden md:inline`), (b) Nummer als JSX-Prop reinreichen, (c) o.ä. NICHT nur `flex-col-reverse` (läse 02→01). Nur `ItemDetail` betroffen (Home/ListDetail haben kein Cover).

**Offen (Designfrage, unbeantwortet):** Soll „Was kommt" für Filme ein oberes Fenster bekommen (z.B. 60 Tage), oder dürfen weit entfernte Releases (8 Monate) dauerhaft weit hinten stehen? Aktuell: **kein** Limit.
...
1. **Serien · Filme · Spiele integrieren.**
   ...
   - **1b · Filme (TMDB) — ✓ erledigt (in `main`).** Binärer Gesehen-Status (`item_history` `completed` an/aus, keine CHECK-Migration), eigene Film-Detailseite, Mitseher-Auge via Co-Member-RLS, unreleased Filme in „Was kommt" nach DE-Release. Detail → §Status-Zeile *Thema 1b*. Restliche Detailseiten-Politur → „Nächste Session" unten.
   - **1c · Spiele (Steam) — offen, als Nächstes nach der Detailseiten-Politur.** Steam-Store-Endpoints (`store.steampowered.com/api/storesearch` + `/appdetails`, kein API-Key, kein OAuth) — **aber CORS-geblockt → braucht einen Supabase Edge-Function-Proxy** (Deno). `items.source` braucht **Migration** (`'steam'` fehlt in der CHECK; `'igdb'` wäre erlaubt, nehmen wir aber nicht). Status-UI aus 1b wird wiederverwendet. (SteamDB.info selbst geht NICHT — Cloudflare/kein API.)

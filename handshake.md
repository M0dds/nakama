...
Freundes-Feedback in `FEEDBACK-BACKLOG.md` (18 Punkte, R1-R5). **R1-R3 deployed** (v0.7.0–v0.9.0) → **nächste Session startet bei R4** (Interaktion: F7 ganzes Item per Drag&Drop · F5 Add-Item rückgängig · F1 Logbuch-Filter + Paging — ⚠️ Event-Mapping vor dem Bau klären, Backlog §Vor dem Bau). R5 (Listen-Kategorien) braucht eine `lists.category`-Migration + eigene Plan-Runde.

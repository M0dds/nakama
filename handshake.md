...
**Stand (2026-06-04):** Phasen 1-8 + Sync-Instanzen + Politur + alle Medien-Themen (1a Serien · 1b Filme · 1c Spiele/Steam) + First-Login-Setup + Cover-Epos sind in lokalem `main`. Build grün, Tree clean, **27 Migrationen** (alle gefahren + bestätigt; Liste → §Gotchas → Daten/RLS). Arbeit läuft über kurzlebige Feature-Branches → `main` (atomare Commits, „better safe than sorry"); **`origin` bewusst NICHT aktuell** (~200 Commits voraus, „mergen ok, nur nicht pushen"). **Thema 4 (Onboarding-Tooltips) verworfen** — Empty-Canvas-Onboarding löst es besser (§Offene Punkte). **Zuletzt (2026-06-04):** **#15 Security-Audit** (Multi-Agent, 17 Agenten/6 Dimensionen, adversariell verifiziert → 9 bestätigt/2 widerlegt; Report `SECURITY-AUDIT.md`) — kein kritisches Finding, RLS-Schicht sauber; 3 Pre-Launch-Härtungen gefahren (M-1 Katalog-RPC-Authz, M-2 Handle-Integrität, L-1 Längen-Caps; Migr. `20260604100000/110000/120000`). **Nächstes:** Launch-Runway (Phase 9 / E-Mail-Prod) + Security-Deploy-Reste (steam-proxy-Auth, RLS-Baseline-Snapshot) → §Offene Punkte.

**Stand (2026-06-04, abends — Post-Launch-Session):** App ist **live auf usenakama.app**; **`origin/main` ist jetzt aktuell** (die alte „origin nicht aktuell"-Notiz oben ist überholt — Git-Auto-Deploy aus `main`). Heute deployed (alle live, Details → `PRE-LAUNCH-FEATURES.md` §Deploy-Historie): **0.2.0** Pre-Launch-Features (Versionierung/Release-Notes/PWA-Install) · **0.2.1** Install-Hotfix · **0.3.0** Update-Toast · **0.4.0** **Steam-Suche live** (steam-proxy-Edge-Function deployed + CORS-Secret) · **0.4.1** robuster Update-Reload · **0.5.0** **Push-Benachrichtigungen Phase 1** (VAPID + `push_subscriptions` + SW-Handler + `send-push`-Function + Profil-Toggle/Test) · **0.6.0** **stille Updates** (Badge am Profil-Nav statt Toast). **Push end-to-end verifiziert** (die zähe Nicht-Zustellung war Chromes hängende GCM-Verbindung, kein Code-Bug). **Aktiver Backlog: `FEEDBACK-BACKLOG.md`** — Freundes-Feedback, 18 Punkte in 5 Releases (R1–R5) sortiert; **die nächste Session startet bei R1**. Lose Enden: `push_subscriptions` wurde als **rohes SQL** gefahren (kein Migrations-File im Repo → ggf. nachtragen); `send-push`-CORS-Allowlist enthält localhost:4173/5173 (Test); Push **Phase 2** (Auto-Versand bei neuen Folgen / Cron) offen. Lokales `main` ist ein paar **Docs-Commits** vor `origin` (bewusst nicht einzeln gepusht — `__GIT_SHA__` triggert sonst einen Fehl-Update-Badge; reiten mit dem nächsten echten Deploy mit).

**Stand (2026-06-04 — Feedback R1 deployed):** **R1 (Feinschliff & Fixes) ist live als `v0.7.0`** (gepusht → Cloudflare-Auto-Deploy). Inhalt: F2 (Listen-Empty-State angeglichen + Text), F13 (Tracken/Archiv-Beschreibung umbrochen), F6 (Kalender-Monatslabel feste Breite `formatMonthAbbr`), F18 (Episoden-Datum + 2-stelliges Jahr `dateLabelShortYear`), F14 (Home begrüßt mit Name via `myProfileOptions`), F15 (`PageHeader` app-weit sticky), F10 (PWA `theme-color` dynamisch via `paintThemeColor` + neutralisiertes Manifest). On top: **`ContentFrame`-Hairlines erscheinen nur noch > `--content-max`** (gutter-breite `overflow-hidden`-Clipbox statt bündiger Rand-Linie) + verwaiste `PwaUpdater.tsx` entfernt. 9 atomare Commits, ff-merge nach `main`, Branch gelöscht, Build grün. **Backlog-Stand:** R1 ✓ in `FEEDBACK-BACKLOG.md` abgehakt → **nächste Session startet bei R2 (Themes & Theme-Picker: F16 Totoro-Kontrast, F17 Pond-Theme ⚠️Farben, F8 Picker 3/Reihe).**

**Stand (2026-06-04 — Hotfix 0.7.1 deployed):** Nach dem 0.7.0-Deploy meldete ein Freund: stilles Update-Badge kam korrekt, aber der Reload danach lief in „Page can't be reached" (nur per Hard Reload behebbar). **Ursache:** der generierte SW (`registerType:"prompt"`) rief **kein `clients.claim()`** auf → nach `SKIP_WAITING` aktivierte der neue SW + `cleanupOutdatedCaches()` löschte den alten Precache, aber die offene Seite blieb am **alten** Controller (kein `controllerchange`) → `applyUpdate`-3s-Backstop lud in den alten Controller mit gelöschtem Precache → tote Navigation. **Fix (0.7.1, live):** `clientsClaim: true` in der Workbox-Config (`vite.config.ts`) → neuer SW übernimmt sofort → `controllerchange` feuert → Reload unter dem neuen, intakten Precache. `skipWaiting` bleibt false (Prompt-Modus). Greift schon beim 0.7.0→0.7.1-Übergang (der **neue** SW claimt). Live verifiziert (`/sw.js` enthält `clientsClaim`). **`origin/main` ist aktuell** (R1-Docs ritten mit dem 0.7.1-Deploy mit). **Durable Lektion:** Prompt-Mode-PWA + manuelles `SKIP_WAITING` braucht `clientsClaim`, sonst strandet der Update-Reload — Memory: `pwa-clientsclaim-update-reload`. Einzig lokal-offen: dieser handshake-Eintrag (reitet mit R2-Deploy mit, kein Solo-Push wegen Fehl-Update-Badge).
...
- **2026-06-03 (Pager-Bounce-Richtung + „Was kommt"-Collapse + Notizen):** Branch `feat/logbuch-seasons-completions` (Forts.). **Pager-Bounce horizontal/richtungs-abhängig** (vorwärts von rechts, rückwärts von links; `on(page,(p,prev))`). **Mobile „Was kommt"** kriegt vertikalen Reveal (neue Reihe von oben, `<For>`→`<Index>` gegen Remount-Flackern) + **„Weniger anzeigen"**-Button. **Notizen (Section 03):** neues geteiltes Notizbrett pro (list, item) — `item_notes`-Tabelle (Migr. `20260603100000`), `notes.ts`, `ItemNotes`-Komponente, in `ItemDetail` rechts unter Details. Design-Entscheidungen: geteilt mit Listen-Mitgliedern (Sharing-Einheit = Liste, RLS `is_list_member`); Block-Liste (Text/Link); nur mit Listen-Kontext sichtbar (§Gotchas → Notizen-Scope).
- **2026-06-04 (#15 Security-Audit):** Branch `feat/logbuch-seasons-completions` (Forts.). Multi-Agent-Audit (17 Agenten, 6 Dimensionen: RLS · XSS · Auth · Secrets · SSRF · Input; jedes Finding adversariell verifiziert → 11 roh, 9 bestätigt, 2 widerlegt). Report in `SECURITY-AUDIT.md`. **Urteil:** kein kritisches Finding, kein aktives Datenleck/Auth-Bypass; RLS-Schicht verifiziert sauber (alle 4 historischen Leck-Klassen geschlossen). XSS clean (einziger HTML-Sink `GeneratedCover` ist numerisch; `normalizeUrl` blockt `javascript:`/`data:`), `npm audit` = 0, kein `service_role`-Key, nichts in Git-History. **Widerlegt** (Audit-Tiefe): Open-Redirect via `?next=` (Solid Router neutralisiert Scheme/protocol-relative + nutzt `pushState`, kein `window.location`) + steam-proxy-Status-Reflektion (Host gepinnt → kein SSRF). **Drei Härtungs-Migrationen gefahren + bestätigt:** `20260604100000` (M-1: `set_item_metadata`/`upsert_episodes` auf Aufrufer gescopt via neuem `can_write_catalog_item`, **stiller No-Op** statt `raise` — der ungefangene Enrichment-Write in `episodes.ts` darf auf Deep-Links nicht crashen), `20260604110000` (M-2: CHECK `^[a-z0-9._-]{3,30}$` + Unique-Index auf `lower(username)` — Handle-Format war nur client-seitig erzwungen), `20260604120000` (L-1: Längen-CHECKs Liste/Beschreibung/Display-Name/Notiz/URL + `maxlength` an den Inputs). **Forts. 2026-06-04 — Audit-Reste abgearbeitet:** (1) **steam-proxy gehärtet** (`SEC-DEPLOY`): per-User-`getUser()`-Auth (Client sendet jetzt Session-`access_token` statt Anon-Key, steam.ts), CORS-Allowlist (`ALLOWED_ORIGINS`-Secret) statt `*`, Param-Allowlist je Endpoint — Code erledigt, Deploy + Secret bleiben für Phase 9 (Function noch nicht live). (2) **`SEC-BASELINE`**: kompletter Live-Sicherheits-Layer (Funktionen/RPCs/RLS/Policies/Trigger) per SQL-Editor-Introspektion in `supabase/migrations/00000000000000_baseline.sql` snapshottet (Docker-frei, da `supabase db dump` Docker braucht) → Security-Modell aus diesem Repo allein reviewbar; **Tabellen-DDL bewusst offen** (liegt in Logbook-Core + Nakama-ALTERs; voller `pg_dump` später optional). (3) **`SEC-TMDB`** bewusst akzeptiert (low). **Launch-Checkliste** in `PHASE-9-LAUNCH.md`.
- **2026-06-04 — 🚀 LIVE-Deploy.** App läuft auf **https://usenakama.app** (Cloudflare Workers + Static Assets, **Git-Auto-Deploy aus `main`** via `M0dds/nakama` — `main` wurde gepusht, `origin` ist jetzt aktuell). SPA-Fallback über `wrangler.jsonc` (`not_found_handling: single-page-application`; `_redirects` verworfen — neuer Workers-Assets-Validator lehnt Catch-all ab), `.nvmrc`=22, `VITE_*` als **Build**-Variablen in Cloudflare (Runtime-Vars sind bei Assets-only gesperrt), Domain via Porkbun→Cloudflare-Nameserver + Custom Domain am Worker (SSL auto). Supabase-Auth-Redirects auf `usenakama.app` gesetzt, **Discord-Login getestet ✓**. Test-User gelöscht (frischer Start). Details + Resttodos: `PHASE-9-LAUNCH.md` §Erreicht. **Noch offen:** `www` (optional), **steam-proxy-Deploy** (Spiele-Suche in Prod erst danach), E-Mail-Prod/Resend.
- **2026-06-04 — Pre-Launch-Features geplant** (vor Weitergabe an Freunde gewünscht): **Versionierung · Release Notes · PWA-Install-Guide · Push-Notifications** (gephased; iOS-Push setzt installierte PWA voraus → Install-Guide vor Push). Voller Plan + Baufolge + 3 offene Entscheidungen in **`PRE-LAUNCH-FEATURES.md`** — **die nächste Session startet dort.**
...
- **Phase 9 — Deploy/Hosting.** PWA-Manifest steht in `vite.config.ts`; DB-Verifikation (Logbook-Migrationen gegen Live-DB abgleichen + als Nakama-Migrationen tracken). **Steam-Edge-Function deployen** (`supabase/functions/steam-proxy` — dev läuft über den Vite-Proxy, Prod braucht die Function). Function ist seit 2026-06-04 gehärtet (per-User-`getUser()`-Auth, CORS-Allowlist, Param-Allowlist — SEC-DEPLOY): **vor dem Deploy** `npx supabase secrets set ALLOWED_ORIGINS="https://<prod-origin>"` (leer = CORS blockt alles, safe default), dann `npx supabase functions deploy steam-proxy`. Themen 3-4 sind durch/verworfen → Phase 9 + Pre-Launch-Härtung sind jetzt die Runway zum Launch.

-- ============================================================================
-- Continue-watching: season-aware "next episode" + list-consistent new badge.
-- ============================================================================
-- Two fixes that only surfaced once TMDB multi-season series joined the model
-- (AniList anime/manga are always season 1, so episode_number was effectively
-- a global sequence):
--
--   1. "next episode" was min(episode_number) across ALL seasons — meaningless
--      once episode_number resets per season (S2E1, S3E1 …): it collapsed the
--      seasons and returned a bogus number. Now it's the lowest unwatched-
--      released (season_number, episode_number) via DISTINCT ON, and the season
--      is returned too (next_season) so the UI can render "S2 · E03". The
--      client title lookup also needs the season — an episode_number-only match
--      would grab an arbitrary season's title.
--
--   2. has_new_episode ("aired after your last watch") stayed dark for a same-
--      day batch release you'd partially watched — but the /lists badge lit up
--      ("released-but-unwatched within 14 days"), so the two surfaces disagreed.
--      Replaced by new_episode_count: the COUNT of released-but-unwatched
--      episodes in the last 14 days, matching the list engine exactly. The
--      frontend shows the badge when > 0 and pluralizes when > 1 ("Neue Folgen")
--      — no number, just singular/plural. A chronic backlog stays quiet because
--      those episodes aired outside the window.
--
-- Return signature changes (next_episode → next_season+next_episode,
-- has_new_episode → new_episode_count) → drop + recreate. SECURITY INVOKER as
-- before. home_watch_bundles (same prior file) is untouched.
-- ============================================================================
